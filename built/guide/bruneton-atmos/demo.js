...
export default class Demo {
    ...
    init() {
        ...
        Utils.loadTextureData('transmittance.dat', (data) => {
            ...
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.getExtension('OES_texture_float_linear') ? gl.RGBA32F : gl.RGBA16F, TRANSMITTANCE_TEXTURE_WIDTH, TRANSMITTANCE_TEXTURE_HEIGHT, 0, gl.RGBA, gl.FLOAT, data);
        });
        Utils.loadTextureData('scattering.dat', (data) => {
            this.scatteringTexture =
                Utils.createTexture(gl, gl.TEXTURE1, gl.TEXTURE_3D);
            // console.log('scattering tex: ', this.scatteringTexture);
            // console.log('scattering dat: ', data);
            gl.texParameteri(gl.TEXTURE_3D, gl.TEXTURE_WRAP_R, gl.CLAMP_TO_EDGE);
            gl.texImage3D(gl.TEXTURE_3D, 0, gl.RGBA16F, SCATTERING_TEXTURE_WIDTH, SCATTERING_TEXTURE_HEIGHT, SCATTERING_TEXTURE_DEPTH, 0, gl.RGBA, gl.FLOAT, data);
        });
        Utils.loadTextureData('irradiance.dat', (data) => {
            this.irradianceTexture =
                Utils.createTexture(gl, gl.TEXTURE2, gl.TEXTURE_2D);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA16F, IRRADIANCE_TEXTURE_WIDTH, IRRADIANCE_TEXTURE_HEIGHT, 0, gl.RGBA, gl.FLOAT, data);
        });
    ...
    maybeInitProgram() {
        ...
        const gl = this.gl;
        const vertexShader = Utils.createShader(gl, gl.VERTEX_SHADER, this.vertexShaderSource.replace('#version 330', '#version 300 es'));
        const fragmentShader = Utils.createShader(gl, gl.FRAGMENT_SHADER, this.atmosphereShaderSource
    ...
    onRender(atmos) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexBuffer);
        gl.vertexAttribPointer(gl.getAttribLocation(program, 'vertex'), 
        /* numComponents=*/ 2, 
        /* type=*/ this.gl.FLOAT, 
        /* normalize=*/ false, 
        /* stride=*/ 0, 
        ...
        gl.uniformMatrix4fv(gl.getUniformLocation(program, 'view_from_clip'), true, this.viewFromClip);
        gl.uniformMatrix4fv(gl.getUniformLocation(program, 'model_from_view'), true, this.modelFromView);
        ...
        gl.uniform3f(gl.getUniformLocation(program, 'camera'), this.modelFromView[3], this.modelFromView[7], this.modelFromView[11]);
        ...
        gl.uniform3f(gl.getUniformLocation(program, 'earth_center'), 0, 0, -6360000 / kLengthUnitInMeters);
        gl.uniform3f(gl.getUniformLocation(program, 'sun_direction'), Math.cos(this.sunAzimuthAngleRadians) *
            Math.sin(this.sunZenithAngleRadians), Math.sin(this.sunAzimuthAngleRadians) *
            Math.sin(this.sunZenithAngleRadians), Math.cos(this.sunZenithAngleRadians));
        gl.uniform2f(gl.getUniformLocation(program, 'sun_size'), Math.tan(kSunAngularRadius), Math.cos(kSunAngularRadius));
    ...
    setView(viewDistanceMeters, viewZenithAngleRadians, viewAzimuthAngleRadians, sunZenithAngleRadians, sunAzimuthAngleRadians, exposure) {
