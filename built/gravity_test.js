...
Testing.prototype.applyAndAssert = function (pos, vel, acc, mass, expPos, actPos, expVel, actVel, expAcc, actAcc, appendMsg = '') {
    const dt = 1;
...
tests.add('particle at rest', () => {
    ...
    const mass = [1];
    const zeros = [0, 0, 0];
    tests.applyAndAssert(pos, vel, acc, mass, zeros, pos, zeros, vel, zeros, acc);
...
tests.add('particle in motion', () => {
    ...
    const acc = [null, null, null];
    const mass = [1];
    tests.applyAndAssert(pos, vel, acc, mass, [1, 2, 3], pos, [1, 2, 3], vel, [0, 0, 0], acc);
});
